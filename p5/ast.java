    ...
    public boolean typeCheck(){
        ...
        for (DeclNode node : myDecls) {
            if (node instanceof FnDeclNode) {
                boolean tmp = ((FnDeclNode)node).typeCheck();// avoid short circuit
                ret = ret &&  tmp;
                //ret = ret && ((FnDeclNode)node).typeCheck(); // short circuiting
            } else {
                //no need to consider vardecl and strcutdecl 
            }
        }
        return ret;
    }
...
class FnBodyNode extends ASTnode {
    ...
    public boolean typeCheck(IdNode id){
        return myStmtList.typeCheck(id);
    }
...
class StmtListNode extends ASTnode {
    ...
    public boolean typeCheck(IdNode id){
        ...
        for (StmtNode node : myStmts) {
            boolean tmp = ((StmtNode)node).typeCheck(id);// used for return; avoid short circuiting
            ret = ret &&  tmp;
        }
        return ret;
    }
...
class ExpListNode extends ASTnode {
    ...
    public Type typeCheck(IdNode id){// seems this is only use in call stmt
        Type ret = new VoidType();//default to voidtype
        List<Type> types = ((FnSym)id.sym()).getParamTypes();
        for (int i=0;i<myExps.size();i++) {
            Type t = myExps.get(i).typeCheck(id);// recursive
            if(!t.isErrorType()){
            ...
            }//else do nothing, stop at errortype according to the webpage
        }
        return ret;
    }
...
class FnDeclNode extends DeclNode {
    ...
    public boolean typeCheck(){
        return myBody.typeCheck(myId); //myId to get the return type
    }
...
//addedby
    public boolean typeCheck(IdNode id){//id is for function name
        return true;
    }
}
...
class AssignStmtNode extends StmtNode {
    ...
    public boolean typeCheck(IdNode id){//id is for function name
        if(myAssign.typeCheck(id).isErrorType()){
            return false;
        }
        return true;
...
class ReadStmtNode extends StmtNode {
    ...
    public boolean typeCheck(IdNode id){
        ...
        if(t.isErrorType() ){
            //TODO:have print or not? hopefully not
            ret = false;
        }
...
class WriteStmtNode extends StmtNode {
    ...
    public boolean typeCheck(IdNode id){
        ...
        if(t.isErrorType() ){
            //TODO:have print or not? hopefully not;not
            ret = false;
        }
...
class CallStmtNode extends StmtNode {
    ...
    public boolean typeCheck(IdNode id){//id is for function name
        if(myCall.typeCheck(id).isErrorType()){
            return false;
        }
        return true;
    }
...
class ReturnStmtNode extends StmtNode {
    ...
    public boolean typeCheck(IdNode id){
        SemSym sym = id.sym();
        boolean ret = true;
        if (!((FnSym)sym).getReturnType().isVoidType() && myExp==null){ //Missing return value
        ...
        if (!((FnSym)sym).getReturnType().isVoidType() && myExp != null){
            Type t = myExp.typeCheck(id);
            if(!t.equals(((FnSym)sym).getReturnType())){//Bad return value goes here
...
//addedby
    ...
    //for a  ExpNode, typeCheck returns a Type
    public Type typeCheck(IdNode id){//TODO id may be be needed for ExpNode
        return null;
    }
    //public Type getType(){
    //    return null;
