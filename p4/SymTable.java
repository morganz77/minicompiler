...
public class SymTable{
    ...
    private LinkedList<HashMap<String, SemSym>> symList;

    /**
     * Initialize an empty list of hashmap for storing symbols
     */
    public SymTable(){
        symList = new LinkedList<HashMap<String, SemSym>>();
        symList.add(new HashMap<String, SemSym>());
    }
    ...
    public void addScope(){
        this.symList.addFirst( new HashMap<String,SemSym> ());
    }

    /**
     * If the first HashMap in the list contains name as a key, return 
     * the associated SemSym; otherwise, return null
     *
     * @param name the name of the symbol to be looked up
     * @return the symbol associated with the name
     * @throws EmptySymTableException if this SymTable's list is empty
     */
    public SemSym lookupLocal(String name) throws EmptySymTableException{
        if (this.symList.isEmpty() ){
            throw new EmptySymTableException();
        } else if ( this.symList.getFirst().containsKey(name) ){
            return (SemSym)(this.symList.getFirst().get(name));
        } else {
            return null; 
        }
    }
     ...
     * first associated SemSym (i.e., the one from the HashMap that is 
     ...
     * @throws EmptySymTableException if this SymTable's list is empty
     */
    public SemSym lookupGlobal(String name) throws EmptySymTableException{
        if (this.symList.isEmpty() ){
            throw new EmptySymTableException();
        } else {
            for (Map<String, SemSym> m: this.symList) {
